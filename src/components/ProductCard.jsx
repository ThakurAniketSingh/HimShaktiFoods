  ...
  return (
    ...
    >
      <div
        ...
        tabIndex={-1}
        onKeyDown={handleKey}
        className="relative bg-surface rounded-xl2 max-w-[680px] w-full max-h-[90vh]
      ...
      >
        <button onClick={onClose} aria-label="Close"
          className="absolute top-3 right-3 z-10 w-8 h-8 rounded-full bg-earth
            hover:bg-heading/10 flex items-center justify-center text-ink-3
            hover:text-heading transition-colors text-lg leading-none">
          ✕
        </button>
        ...
        <div className="p-5 sm:p-7 flex flex-col gap-5">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              ...
              <h2 className="font-serif text-2xl text-heading leading-snug">{product.name}</h2>
              <p className="text-xs text-amber font-bold uppercase tracking-widest mt-1">{product.category}</p>
            </div>
            <div className="text-right shrink-0">
              <p className="font-serif text-3xl text-heading">₹{product.price}</p>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {product.ingredients.map((ing) => (
                  <span key={ing}
                    className="text-[12px] bg-earth text-ink-2 px-2.5 py-1 rounded-full border border-edge/8">
                    {ing}
                  </span>
                ))}
              </div>
          ...
          {loading ? (
            ...
            <div className="flex items-center justify-center gap-2.5 bg-red-500 cursor-not-allowed
...
export default function ProductCard({ product }) {
  ...
  return (
    <>
      <article
        className="group flex flex-col bg-surface rounded-xl2 overflow-hidden border border-edge/8
          shadow-sm hover:shadow-xl hover:-translate-y-1.5 hover:border-edge/20
      ...
      >
        <div className="relative flex items-center justify-center h-[168px] overflow-hidden"
          ...
          <div className="absolute top-3 left-3 flex gap-1.5 flex-col items-start sm:flex-row">
            {product.onSale && (
        ...
        <div className="flex flex-col flex-1 p-4 gap-2.5">
          ...
          <h3 className="font-serif text-[1.1rem] text-heading leading-snug">
            {product.name}
          </h3>
          ...
          <div className="flex items-center justify-between pt-2.5 border-t border-edge/6 mt-0.5">
            <span className="text-[11px] font-medium bg-earth text-ink-3 px-2.5 py-1 rounded-full">
              ⚖️ {product.weight}
            </span>
            <span className="font-serif text-xl text-heading">₹{product.price}</span>
          </div>
          ...
          {loading ? (
            ...
            <div
              onClick={(e) => e.stopPropagation()}
              className="flex items-center justify-center gap-2 bg-red-500 cursor-not-allowed
